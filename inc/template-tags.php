...
function stanleywp_posted_on() {
	...
	if ( 'post' === get_post_type() ) {
		...
		if ( $categories_list && stanleywp_categorized_blog() ) {
			printf( '<span class="cat-links">' . esc_html__( '%1$s', 'stanleywp' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}
	}
...
function stanleywp_entry_footer() {
	...
	//if ( 'post' === get_post_type() ) {
		...
		// if ( $tags_list ) {
		// 	printf( '<span class="tags-links">' . esc_html__( '%1$s', 'stanleywp' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		// }
	//}
